...
class Preferences:
    ...
    def most_preferred(self, item_list):
        ...
        for item in item_list :
            scores.append(item.get_score(self))
        ind = np.argmax(scores)
        best_item = item_list[ind]
    ...
    def is_item_among_top_10_percent(self, item, item_list):
        ...
        # To be completed        
        top_ten = []
        num = int(np.ceil(len(item_list) / 10))
        if num == 0 :
            return False
        for _ in range(num) :
            it = self.most_preferred(item_list)
            top_ten.append(it)
            item_list.remove(it)
...
if __name__ == '__main__':
    ...
    print('{} is in top ten : {}'.format(diesel_engine.get_name(), agent_pref.is_item_among_top_10_percent(diesel_engine, [diesel_engine, electric_engine])))